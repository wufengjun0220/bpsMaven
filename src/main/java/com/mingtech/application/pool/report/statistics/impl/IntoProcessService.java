...
public class IntoProcessService extends GenericServiceImpl{
	...
	public void txProcess(Date startDate,Date endDate) throws Exception{
		...
		if(processList1 != null){
			...
			logger.info("-------------------开始查询、分析报表数据，开始时间: " + DateUtils.toString(new Date(), DateUtils.ORA_DATE_TIMES3_FORMAT));
			/*if(bankLevel.equals("3")){ // 支行级别
				...
				if(!deptList.isEmpty()){
					this.statisticsByDept(startDate, endDate, deptList, bankLevel);
				}
				...
				logger.info("成功查询、分析月报表！结束时间：" + DateUtils.toString(new Date(), "yyyy年MM月dd日 HH时mm分ss秒"));
//			}
			logger.info("-------------------查询、分析报表数据结束，结束时间: " + DateUtils.toString(new Date(), DateUtils.ORA_DATE_TIMES3_FORMAT));
