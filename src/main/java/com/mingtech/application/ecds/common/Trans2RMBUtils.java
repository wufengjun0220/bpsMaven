...
public class Trans2RMBUtils{
	...
	/**
	 * 将用户输入的字符串数据转换化为金额形式的字符串 <p>方法名称: getRMBStr|描述: </p>
	 * @param str
	 * @return 如果用户输入数据合法，返回字符串，否则返回空字符串
	 */
	public static String getRMBStr(String str){
		if(checkNum(str)){
			String s = cleanZero(splitNum(roundString(str)));
			return s;
		}else{
			return "";
		}
	...
	private static String cleanZero(String s){
		...
		//2010-10-26廊坊本地修改 增加 107000.53 人行规范要求写法
		boolean flag =  s.indexOf("零元")== -1 ? false : true ; //判断个位是否为零元，用于后面判断是否需要加零
		...
		//2010-10-21廊坊本地增加用于改正类似1000.04的错误，正确显示《零四分》&&  增加 107000.53 人行规范要求写法
		if(!s.endsWith("零角零分")){
			if(s.indexOf("零角")!= -1){
				s = s.replace("零角", "零");
			}else if(s.length()>5 && flag){
				int pos = s.indexOf("元");
				s = s.substring(0,pos+1)+"零"+s.substring(pos+1); // 增加 107000.53 人行规范要求写法：《壹拾万柒仟元零伍角叁分》|| 《壹拾万零柒仟元伍角叁分》
			}
		}
		// 第三轮转换把"零角","零分"字符串省略
		for(int i = 0; i < 2; i++){
		...
		s = s.replaceAll("亿万", "亿");
		if(s.endsWith("元")||s.endsWith("角"))s=s+"整";
		return s;
		
	}
	
	/**
	 * 把用户输入的数调用 splitPageNum() 方法 进行相应的中文金额大写形式的转换 注：传入的这个数应该是经过
	 * 是一个String类型的整数
	 * @param s String
	 * @return 转换好的中文金额大写形式的字符串
	 */
	public static String splitPageNum(String s){
		// 如果传入的是空串则继续返回空串
		if("".equals(s)){
			return "";
		}
